...
static void SSD1306_I2C_WriteMulti(SSD1306_Name* SSD1306, uint8_t address, uint8_t reg, uint8_t* data, uint16_t count)
{
	...
	i2c_burst_send(address, TxBuff, count + 1);
}
...
static void SSD1306_I2C_Write(SSD1306_Name* SSD1306, uint8_t address, uint8_t reg, uint8_t data)
{
	...
	TxBuff[0] = reg;
	TxBuff[1] = data;
	uint8_t ret = i2c_burst_send(address, TxBuff, 2);
	if(ret == 0)
	{
		printf("Write data %d success to i2c\r\n", TxBuff);
	}
	else printf("Write data %d failed to i2c, err:%d\r\n", TxBuff, ret);
	/*ret = i2c_single_send(address, TxBuff[1]);
	if(ret == 0)
	{
		printf("Write data %d success to i2c\r\n", TxBuff[1]);
	}
...
uint8_t SSD1306_Init(SSD1306_Name* SSD1306) {
	...
	/* Init I2C of cc2538 */
	i2c_init(SSD1306->I2C_Config.SDA_port, SSD1306->I2C_Config.SDA_pin, SSD1306->I2C_Config.SCL_port, SSD1306->I2C_Config.SCL_pin, SSD1306->I2C_Config.bus_speed);
